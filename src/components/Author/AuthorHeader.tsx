  ...
  return (
    <div className="relative bg-slate-500 flex flex-col-reverse items-center p-4 mt-8 md:px-8 md:flex-row md:justify-between max-w-[1200px] mx-auto overflow-hidden rounded-md">
      <div className="flex items-center gap-2 flex-wrap justify-center">
        ...
        { auth && auth.user.favouriteAuthors?.some(author => author === username) ?
          <button className="flex items-center" onClick={ toggleFavouriteAuthor }><AiFillHeart className="text-red-600 text-2xl"/></button> :
          <button className="flex items-center" onClick={ toggleFavouriteAuthor }><AiFillHeart className="text-2xl text-white"/></button> }
      </div>
